...
////////////////////////////////////////////////////////////////////////////////
int THOR_VT_HandPlugin::GetObjectDetection(
  ...
  double _prevCommand)
{
  ...
  wasMoving[_index] = isMoving;
	...
	////////////////////////////////////////////////////////////////////////////////
	// Load the controller
	void THOR_VT_HandPlugin::Load(physics::ModelPtr _parent,
		sdf::ElementPtr _sdf)
	{
		this->model = _parent;
		...
		gzlog << "THOR_VT_HandPlugin loading for " << this->prefix << " hand." <<
    			std::endl;
		...
		this->deferredLoadThread = boost::thread(
			boost::bind(&THOR_VT_HandPlugin::DeferredLoad, this));
	}
	...
	void THOR_VT_HandPlugin::DeferredLoad()
	{
		...
		for (int finger = 0; finger < NUM_FINGERS; finger++)
		{
			...
			{
				char joint_ns[200] = "";
				snprintf(joint_ns, sizeof(joint_ns), "thor_vt_hands/gains/%s_f%d_j%d/",
					this->prefix.c_str(), finger, joint);
		...
		ros::SubscribeOptions jointCommandsSo =
            ros::SubscribeOptions::create<vigir_grasp_msgs::JointCommands>(
			topic_base + std::string("_hand/joint_commands"), 100,
			boost::bind(&THOR_VT_HandPlugin::SetJointCommands, this, _1),
		...
		this->callbackQueeuThread = boost::thread(
			boost::bind(&THOR_VT_HandPlugin::RosQueueThread, this));

		this->updateConnection = event::Events::ConnectWorldUpdateBegin(
			boost::bind(&THOR_VT_HandPlugin::UpdateStates, this));
	}
...
//ROS_ERROR("%s ; q_p %f ; qd_p %f ; q_i %f d_q_p_dt %f dt %f ; force %f", this->jointCommands.name[i].c_str(), this->errorTerms[i].q_p, this->errorTerms[i].qd_p, this->errorTerms[i].q_i, this->errorTerms[i].d_q_p_dt, dt, force);
	...
	void THOR_VT_HandPlugin::CopyVectorIfValid(const std::vector<double> &from,
	...
	////////////////////////////////////////////////////////////////////////////////
	// Set Joint Commands
	void THOR_VT_HandPlugin::SetJointCommands(
